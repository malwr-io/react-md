...
function reducer(state: SliderState, action: SliderAction): SliderState {
  ...
  switch (action.type) {
    ...
    case "stop":
      if (dragType === null && !state.dragging) {
        return state;
      }
...
export function useSlider({
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (value % step !== 0) {
      throw new RangeError(
        "A slider's value must be divisible by the provided step"
      );
    }
  }
  ...
  useEffect(() => {
    if (!onValueChange || prevValue.current === value) {
      return;
    }

    if (mode === "onBlur" && (dragging || dragType)) {
      return;
    }

    prevValue.current = value;
    onValueChange(value);
  }, [mode, value, dragging, dragType, onValueChange]);
